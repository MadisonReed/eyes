...
eyes.inspect = function (obj, label, options) {
    ...
    if (options.stream) {
        ...
        return stringify(obj, options) + (options.styles ? '\x1B[39m' : '');
    }
};
...
eyes.print = function (str, label, options) {
    ...
    return options.stream.write.call(options.stream, (label ?
        ...
        this.stylize(str,   options.styles.all, options.styles) + '\x1B[0m' + "\n");
};
...
function stringify(obj, options) {
    ...
    result = (function (obj) {
        switch (typeOf(obj)) {
            ...
            case "set"      : return stringifySet(obj, options, stack.length);
            case "map"      : return stringifyMap(obj, options, stack.length);
            case "weakset"  : return stringifyWeakSet(obj, options, stack.length);
            case "weakmap"  : return stringifyWeakMap(obj, options, stack.length);
        }
    })(obj);
...
function stringifyWeakSet(set, options, level) {
    var out = [];
    var pretty = options.pretty && set.size > 4;
    var ws = pretty ? '\n' + new(Array)(level * 4 + 1).join(' ') : ' ';

    for (var key of set.values()) {
        out.push(stringify(key, options));
    }
...
function stringifyWeakMap(map, options, level) {
    var out = [];
    var pretty = options.pretty && map.size > 2;
    var ws = pretty ? '\n' + new(Array)(level * 4 + 1).join(' ') : ' ';

    for (var obj of map.entries()) {
        out.push(obj[0] + ': ' + stringify(obj[1], options));
    }
...
function stringifySet(set, options, level) {
    var out = [];
    var pretty = options.pretty && set.size > 4;
    var ws = pretty ? '\n' + new(Array)(level * 4 + 1).join(' ') : ' ';
...
function stringifyMap(map, options, level) {
    var out = [];
    var pretty = options.pretty && map.size > 2;
    var ws = pretty ? '\n' + new(Array)(level * 4 + 1).join(' ') : ' ';
...
function typeOf(value) {
    var s = typeof(value),
        types = [Object, Array, String, RegExp, Number, Function, Boolean, Date, Set, Map, WeakSet, WeakMap];
